...
const Checkout:React.FC = () => {
    ...
    const [products, setProducts] = useState<PropsDataProducts[]>([])

    const token = localStorage.getItem('token');


    const getProductsOfCart = async () => {
        const res = await axios.get(`${URL_CHECKOUT}/`, 
            {
                headers: {
            ...
            <div className='p-5 gap-5 min-w-96 w-[48rem] h-fit flex flex-col bg-white rounded-lg border border-slate-100'>
                ...
                <div className='flex flex-col gap-1 border-b pb-5 border-b-slate-200'>
                    {products.map((product) => (
                        <div className='flex gap-2 items-center' key={product.id_product}>
